...
EVAL_EPISODE = 50
BATCH_SIZE = 128
PREDICT_BATCH_SIZE = 30 #15     # batch for efficient forward
SIMULATOR_PROC = 100 #50
PREDICTOR_THREAD_PER_GPU = 6
...
def get_player(train=False, dumpdir=None):  # Setting up new gym env() for a player
    env = gym.make(ENV_NAME)
    if dumpdir:
        env = gym.wrappers.Monitor(env, dumpdir, video_callable=lambda _: True)
    env = FireResetEnv(env) # in case the env  needs to be started by firing eg/ breakout
    env = MapState(env, lambda im: cv2.resize(im, IMAGE_SIZE)) # cropping input to IMGAE_SIZE (84x84 standard) 
    env = FrameStack(env, 4) # Stacking 4 recent frames into 1 frame (like DQN) 
    if train:
        env = LimitLength(env, 60000) # max 60k steps per episode in '-v0' envs
    return env


class MySimulatorWorker(SimulatorProcess):
    def _build_player(self):
        return get_player(train=True)   # Takes the player made by get_player and sets train to true


class Model(ModelDesc): # Creating a model structure 
    def inputs(self):   # Inputs for model structure 
        assert NUM_ACTIONS is not None # cannot have 'none' actions for an env()
    ...
    def _get_NN_prediction(self, image):
        image = tf.cast(image, tf.float32) / 255.0  # Casts image as new tensor with destination-type tf.float32
        with argscope(Conv2D, activation=tf.nn.relu):   # gives context to default arguments - applies activation type of tf.nn.relu to all Conv2D types
            l = Conv2D('conv0', image, 32, 5)
            l = MaxPooling('pool0', l, 2)
            l = Conv2D('conv1', l, 32, 5)
            l = MaxPooling('pool1', l, 2)   # creatin nn layers 
    ...
    def build_graph(self, state, action, futurereward, action_prob):
        logits, value = self._get_NN_prediction(state)  # calling get_NN_prediction on game state
        value = tf.squeeze(value, [1], name='pred_value')  # (B,)   # removes dimension of size 1 from the value tensor in 1st list pos (2nd element)  
        policy = tf.nn.softmax(logits, name='policy')   # tensor called Policy will sum to 1 and can be interpreted as containing probabilities
        is_training = get_current_tower_context().is_training # returns boolean 'is_training' for tower_context 
        if not is_training:
            return # return if not training
        log_probs = tf.log(policy + 1e-6) # generates natural log for probabilities (softmax policy) 

        log_pi_a_given_s = tf.reduce_sum(
            log_probs * tf.one_hot(action, NUM_ACTIONS), 1) # 
        advantage = tf.subtract(tf.stop_gradient(value), futurereward, name='advantage') # stops gradient computation on value and subtracts futurereward 

        pi_a_given_s = tf.reduce_sum(policy * tf.one_hot(action, NUM_ACTIONS), 1)  # (B,)
        importance = tf.stop_gradient(tf.clip_by_value(pi_a_given_s / (action_prob + 1e-8), 0, 10)) #stops gradient computation for backprop
    ...
    def optimizer(self):
        lr = tf.get_variable('learning_rate', initializer=0.001, trainable=False) # created new tf var 'learning rate' set at initializer value
...
def train():
    ...
    config = TrainConfig(
        ...
        max_epoch=250, # Max 1.5 million steps
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--episode', help='number of episode to eval', default=88, type=int) #sample size of 88 due to .95 statistical power
    ...
    if args.task != 'train':
        ...
        pred = OfflinePredictor(PredictConfig(
            ...
            output_names=['policy']))
        if args.task == 'play':
            play_n_episodes(ENV_NAME, get_player(train=False), pred,
